...
LinearEquation::LinearEquation(QWidget * parent) : QMainWindow(parent), m_interface(new Ui::LinearEquation)
{
...
{
    for(size_t i = 0; i < m_interface->matrix->rowCount(); ++i)
    {
        for(size_t j = 0; j < m_interface->matrix->columnCount(); ++j)
        {
            ...
            j == m_interface->matrix->columnCount() - 1 ? item.setBackgroundColor(m_colors[VECTOR]) 
                                                        : item.backgroundColor() != m_colors[MEMBERS] ?
                                                          item.setBackgroundColor(m_colors[MEMBERS])  : item;
        }
    }
}

LinearEquation::Errors LinearEquation::ReadTable()
{
    if(!m_scale)
    {
        return CLEAR_TABLE;
    ...
    for(size_t i = 0, j = 0; i < m_scale; ++i, j = 0)
    {
        for(; j < m_scale; ++j)
        {
...
double_t LinearEquation::FindDeterminant() const
{
    ...
    {
        det *= (pivots(i) != i ? -1. : 1.) * temporaryMatrix(i, i);
    }

    return det;
}

double_t LinearEquation::FindDeterminant(ublas::matrix<float_t> & matrix) const
{
...
QVector<double_t> LinearEquation::MakeSolution()
{
    ...
    QScopedPointer<QThread> getResult(QThread::create([this, &result, determinant]
    {
    ...
    getResult->start(QThread::Priority::HighPriority);
...
void LinearEquation::on_actionSettings_triggered()
{
    ...
    connect(&window, SIGNAL(changeSize(int32_t)),             this, SLOT(changeMatrixSize(int32_t)));
    connect(&window, SIGNAL(determinantOnly(Qt::CheckState)), this, SLOT(determinantOnlyChecked(Qt::CheckState)));
    window.exec();
}
...
void LinearEquation::on_actionClean_triggered()
{
    ...
    {
        qFill(m_matrix.begin1(), m_matrix.end1(), 0.0f);
        qFill(m_matrix.begin2(), m_matrix.end2(), 0.0f);
    });
    ...
    } catch(Errors codeError)
    {
        switch(codeError)
        {
            case ZERO_DETERMINANT:
                output = "The determinant is equal to zero!";
                break;
            case CLEAR_TABLE:
                output = "You should fill the table before!";
                break;
            case BAD_DATA:
                output = "Incorrect input in the table!";
                break;
        }
...
void LinearEquation::MakeColumnReadOnly(uint8_t column) const
{
    for(size_t i = 0; i < m_interface->matrix->rowCount(); ++i)
    {
...
void LinearEquation::MakeColumnWriteable(uint8_t column) const
{
    for(size_t i = 0; i < m_interface->matrix->rowCount(); ++i)
    {
